...
What is pinned, and the failure each one guards:
  ...
  6. The per-pose split exposes a degenerate answerer: always saying
     one face scores above the floor overall and must show as 100/0/0.
...
def build_captures(tmp):
    ...
    n01_B is deliberately a LARGE-FACE block whose id ends in _B, which is
    the case a filename rule gets wrong.

    The BLOCK on both of its faces, plus one capture of the face the
    design withdrew, which load_scenes must skip. The mustard is the
    pilot object,
    with two poses and no face vocabulary, and two members would miss the
    pair the design turns on.
    """
    recs = []
    for seq, prim, kind in (("p01_A", "ycb_block_large", "pair"),
                            ("p01_B", "ycb_block_upright", "pair"),
                            ("p01_C", "ycb_block_small", "pair"),
                            ("n01_B", "ycb_block_large", "null")):
...
rows = S.probes(S.load_scenes(TMP, present_ur=False), S.VIEWS)

# 1. truth from the registry, not the filename
truth = {r["seq"]: r["true_pose"] for r in rows}
faces = {r["seq"]: r["true_face"] for r in rows}
check("one probe per scene per view, retired faces skipped",
      len(rows) == 6,
      "%d rows: p01_C rests on the middle face, withdrawn 2026-08-27, "
      "and must not be probed" % len(rows))
check("truth comes from the registry entry, not the _B suffix",
      truth["p01_B"] == "small_face" and truth["n01_B"] == "large_face",
      "n01_B ends in _B and is a large-face null")
check("the deployed pose is translated into the answer vocabulary",
      faces["p01_A"] == "large_face" and faces["p01_B"] == "small_face",
      "the two vocabularies CROSS on the word small, so an untranslated "
      "comparison scores every correct answer wrong")
check("the withdrawn face is not probed at all",
      "p01_C" not in faces,
      "its capture is still on disk and must be skipped, not scored")

# 2, 3, 4. parsing
sees, pose = S.parse("SEES: it is much wider than it is tall\n"
                     "FACE: large_face")
check("both lines are read",
      pose == "large_face" and sees.startswith("it is much"))
sees, pose = S.parse("SEES: it looks like the small_face to me\n"
                     "FACE: large_face")
check("the face comes from the FACE line, not the description",
      pose == "large_face", "description said small_face")
...
sees, pose = S.parse("I think it is resting on the large face.")
...
_pairs = S.probes(S.load_scenes(TMP, present_ur=False), ("ex2_cam",), kind=S.DEFAULT_KIND)
_faces = [r["true_face"] for r in _pairs]
check("the default kind drops the nulls",
      all(r["kind"] == "pair" for r in _pairs), "%d scenes" % len(_pairs))
check("pairs give equal numbers of each face",
      set(_faces) == set(S.ANSWERS)
      and len({_faces.count(f) for f in set(_faces)}) == 1,
      "counts %s" % {f: _faces.count(f) for f in sorted(set(_faces))})
_all = S.probes(S.load_scenes(TMP, present_ur=False), ("ex2_cam",), kind=None)
_all_faces = [r["true_face"] for r in _all]
check("without the filter the sample is lopsided",
      _all_faces.count("large_face") > _faces.count("large_face"),
...
check("the default view is the oblique camera alone",
      ...
      "the overhead camera scored at the floor and has no cue to probe")
...
_tbl = S.table([
    {"seq": "s1", "repeat": 0, "model": "a", "view": "ex2_cam",
     "true_pose": "large_face", "true_face": "large_face",
     "pose": "large_face", "correct": True},
    {"seq": "s2", "repeat": 0, "model": "a", "view": "ex2_cam",
     "true_pose": "small_face", "true_face": "small_face",
     "pose": "large_face", "correct": False},
    {"seq": "s3", "repeat": 0, "model": "b", "view": "ex2_cam",
     "true_pose": "small_face", "true_face": "small_face",
     "pose": None, "correct": None},
])
...
check("a terse reply is parsed as a bare word with no description",
      S.parse("small_face", "terse") == (None, "small_face"))
...
# A cell whose whole interval sits under chance is not merely
# uninformative: the view is inverting the cue. The example has moved with
# the probe twice now -- two-way at 50, three-way at 33.3, and two-way
# again since 2026-08-27 -- so the rates are chosen against S.CHANCE rather
# than written as literals, which would pin an arithmetic identity instead
# of the claim.
_lo, _hi = S.wilson(3, 30)
check("a cell below chance is recognisable as one",
      _hi < S.CHANCE,
      "3/30 upper bound %.1f is under %.1f" % (_hi, S.CHANCE))
_lo, _hi = S.wilson(15, 30)
check("a cell straddling chance is not called below it",
      _hi >= S.CHANCE,
      "15/30 upper bound %.1f reaches %.1f, so it is uninformative rather "
      "than inverted" % (_hi, S.CHANCE))

# 13. repeats collapse to ONE verdict per image before the interval.
check("an image's repeats are settled by majority",
      S.majority(["large_face", "large_face", "small_face"]) == "large_face")
check("a tied image is unusable, not silently resolved",
      S.majority(["large_face", "small_face"]) is None,
...
_reps = [{"model": "m", "format": "terse", "view": "v", "seq": "s1",
          "true_pose": "large_face", "true_face": "large_face",
          "pose": p, "correct": p == "large_face", "repeat": i + 1}
         for i, p in enumerate(["large_face", "large_face", "small_face"])]
